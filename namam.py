...
while True:
    ...
    for output in layer_outputs:
        for detection in output:
            ...
            if confidence > 0.5:
                ...
                elif class_id == 17:  # 17 represents the 'cat' class
                    cat_boxes.append([x, y, w, h])

                confidences.append(float(confidence))
                class_ids.append(class_id)

    # Check if a person and cat are detected at the same time
    person_detected = len(person_boxes) > 0
    cat_detected = len(cat_boxes) > 0
    if person_detected and cat_detected:
        person_box = person_boxes[0]  # Assuming only one person and one cat are detected
        cat_box = cat_boxes[0]
    ...
    if person_detected or cat_detected:
        if person_detected and cat_detected:
            csv_writer.writerow([timestamp, 'Person and Cat'])
        elif person_detected:
            csv_writer.writerow([timestamp, 'Person'])
        else:
            csv_writer.writerow([timestamp, 'Cat'])
    ...
    indices = cv2.dnn.NMSBoxes(person_boxes + cat_boxes, confidences, 0.5, 0.4)
    ...
    for i in indices:
        ...
        if i < len(person_boxes):
            ...
            x, y, w, h = cat_boxes[i - len(person_boxes)]
            # Process cat detection here if needed
